...
#     assert seg.point == Point(1, 0)
#     assert seg.direction == Direction.H
#     assert seg.length == 1
#
#     seg = conn.segments[2]
...
def test_straight_connectors_without_body():
    ...
    assert conn.segments[1] == Terminal(Point(1, 0), TerminalType.END)

    conn = Connector(Point(0, 0), Point(0, 1))
    assert len(conn.segments) == 2
    assert conn.segments[0] == Terminal(Point(0, 0), TerminalType.START)
    assert conn.segments[1] == Terminal(Point(0, 1), TerminalType.END)

    conn = Connector(Point(1, 0), Point(0, 0))
    assert len(conn.segments) == 2
    assert conn.segments[0] == Terminal(Point(1, 0), TerminalType.START)
    assert conn.segments[1] == Terminal(Point(0, 0), TerminalType.END)
...
def test_straight_connectors_with_body():
    ...
    assert conn.segments[2] == Terminal(Point(2, 0), TerminalType.END)
    ...
    assert conn.segments[2] == Terminal(Point(0, 0), TerminalType.END)

    conn = Connector(Point(0, 0), Point(5, 0))
    assert len(conn.segments) == 3
    assert conn.segments[0] == Terminal(Point(0, 0), TerminalType.START)
    assert conn.segments[1] == Line(Point(1, 0), Direction.H, 4, True)
    assert conn.segments[2] == Terminal(Point(5, 0), TerminalType.END)
    ...
    assert conn.segments[2] == Terminal(Point(0, 2), TerminalType.END)

    conn = Connector(Point(0, 0), Point(0, 5))
    assert len(conn.segments) == 3
    assert conn.segments[0] == Terminal(Point(0, 0), TerminalType.START)
    assert conn.segments[1] == Line(Point(0, 1), Direction.V, 4, True)
    assert conn.segments[2] == Terminal(Point(0, 5), TerminalType.END)
